...
from models.motus_wan_vlm_direct_mask_reweight_progress import MotusWanVlmDirectMask, MotusWanVlmDirectMaskConfig
...
class MotusWanVlmTrainer:
    ...
    def train_step(self, batch: Dict[str, Any]) -> Dict[str, float]:
        ...
        # Subtask prompts and progress targets
        subtask_prompts = batch.get('subtask_prompts')
        subtask_lm_inputs = batch.get('subtask_lm_inputs')
        if subtask_lm_inputs is not None:
            subtask_lm_inputs = {k: v.to(self.device) if isinstance(v, torch.Tensor) else v
                                for k, v in subtask_lm_inputs.items()}
        subtask_input_ids = batch.get('subtask_input_ids')
        if subtask_input_ids is not None:
            subtask_input_ids = subtask_input_ids.to(self.device)
        subtask_labels = batch.get('subtask_labels')
        if subtask_labels is not None:
            subtask_labels = subtask_labels.to(self.device)
        progress_targets = batch.get('progress_targets')
        ...
        loss_dict = model.training_step(
            ...
            subtask_lm_inputs=subtask_lm_inputs,
            subtask_input_ids=subtask_input_ids,
        ...
        # Gradient NaN check (first 5 steps only)
        if not hasattr(self, '_grad_nan_check_step'):
            self._grad_nan_check_step = 0
        if self._grad_nan_check_step < 5:
            self._grad_nan_check_step += 1
            nan_params = []
            inf_params = []
            # Also track gradient norm per group for diagnosis
            grad_norms = {}
            for name, p in model.named_parameters():
                if p.grad is not None:
                    gnorm = p.grad.norm().item()
                    if any(an in name for an in ['progress_mlp']):
            ...
            if grad_norms:
                logger.info(f"GRAD NORMS [step {self._grad_nan_check_step}] aux heads: {grad_norms}")
            if nan_params:
                logger.warning(f"GRAD NaN CHECK [step {self._grad_nan_check_step}]: NaN gradients in {len(nan_params)} params, first 5: {nan_params[:5]}")
            if inf_params:
                logger.warning(f"GRAD NaN CHECK [step {self._grad_nan_check_step}]: Inf gradients in {len(inf_params)} params, first 5: {inf_params[:5]}")
            if not nan_params and not inf_params:
                logger.info(f"GRAD NaN CHECK [step {self._grad_nan_check_step}]: All gradients OK")
        ...
        # Weight health check (first 5 steps, after optimizer step)
        if self._grad_nan_check_step <= 5:
            weight_issues = []
            for name, p in model.named_parameters():
                if any(an in name for an in ['progress_mlp']):
                    if torch.isnan(p).any() or torch.isinf(p).any():
                        weight_issues.append(f"{name}: nan={torch.isnan(p).sum().item()}, inf={torch.isinf(p).sum().item()}")
            if weight_issues:
                logger.warning(f"WEIGHT CHECK [step {self._grad_nan_check_step}]: {weight_issues}")
    ...
    def log_metrics(self, loss_dict: Dict[str, float], elapsed_time: float):
        ...
        if self.rank == 0:
            # Build loss string
            loss_parts = [f"Video: {loss_dict['video_loss']:.4f}", f"Action: {loss_dict['action_loss']:.4f}"]
            if 'subtask_loss' in loss_dict:
                loss_parts.append(f"Subtask: {loss_dict['subtask_loss']:.4f}")
            if 'progress_loss' in loss_dict:
                loss_parts.append(f"Progress: {loss_dict['progress_loss']:.4f}")
            loss_str = ", ".join(loss_parts)

            logger.info(
                f"Step {self.global_step}/{self.config.training.max_steps} (Epoch {self.epoch}), "
                f"Loss: {loss_dict['total_loss']:.4f} ({loss_str}), "
                f"LR(main/wan/vlm)={lr_main:.2e}/{lr_wan:.2e}/{lr_vlm:.2e}, "
                f"Time: {elapsed_time:.2f}s"
            )

            # Log subtask and progress values every 20 steps
            if self.global_step % 20 == 0:
                if 'subtask_loss' in loss_dict:
                    decoded_pred = loss_dict.get('decoded_subtask', '')
                    decoded_target = loss_dict.get('subtask_target', '')
                    subtask_prompt = loss_dict.get('subtask_prompt', '')
                    logger.info(f"  [Subtask] loss: {loss_dict['subtask_loss']:.4f}")
                    if decoded_target:
                        logger.info(f"  [Subtask] target: {decoded_target}")
                    if decoded_pred:
                        logger.info(f"  [Subtask] pred: {decoded_pred}")
                    if subtask_prompt:
                        logger.info(f"  [Subtask] prompt: {subtask_prompt}")
                if 'progress_pred_mean' in loss_dict:
                    logger.info(
                        f"  [Progress] pred={loss_dict['progress_pred_mean']:.4f}, "
                        f"target={loss_dict['progress_target_mean']:.4f}"
                    )
            ...
            # Log to TensorBoard - only numeric loss_dict metrics
...
def create_model_and_optimizer(config: OmegaConf) -> tuple:
    ...
    model_config = MotusWanVlmDirectMaskConfig(
        ...
        subtask_prediction=dict(config.model.subtask_prediction) if hasattr(config.model, 'subtask_prediction') else None,
        progress_detection=dict(config.model.progress_detection) if hasattr(config.model, 'progress_detection') else None,
    )
    ...
    # Auxiliary heads (progress MLP) use 1/10 of base LR
    # to prevent large CE loss gradients from causing weight explosion
    aux_lr = float(getattr(config.training, 'aux_learning_rate', base_lr * 0.1))
    ...
    # Separate auxiliary head parameters (progress MLP, subtask decoder) from other_params
    aux_param_names = ['progress_mlp_in', 'progress_mlp_out',
                       'subtask_text_decoder', 'subtask_action_memory_proj', 'subtask_vlm_memory_proj']
    aux_param_ids = set()
    for name, p in model.named_parameters():
        if p.requires_grad and any(an in name for an in aux_param_names):
            aux_param_ids.add(id(p))

    # Other params are Action Expert + Qwen3 Module (projections), excluding aux heads
    other_params = [p for p in all_trainable
                    if id(p) not in wan_param_ids and id(p) not in vlm_param_ids and id(p) not in aux_param_ids]
    aux_params = [p for p in all_trainable if id(p) in aux_param_ids]
    ...
    if len(aux_params) > 0:
        param_groups.append({'params': aux_params, 'lr': aux_lr})
        logger.info(f"Created aux param group with {len(aux_params)} params, lr={aux_lr}")
...
def main():
    ...
    try:
        ...
        if getattr(config.model, 'training_mode', 'finetune') == 'finetune' and finetune_ckpt:
