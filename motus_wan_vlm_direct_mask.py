...
class MotusWanVlmDirectMaskConfig:
    ...
    action_dim: int = 14
    action_padding_dims: tuple = ()  # Dimensions that are zero-padded (excluded from action loss), e.g. (6, 14) for 16-dim
...
class MotusWanVlmDirectMask(nn.Module):
    ...
    def __init__(self, config: MotusWanVlmDirectMaskConfig):
        ...
        if load_backbones:
            ...
            logger.info("Load pretrained VLM (flash_attention_2)...")
        else:
            vlm_cfg = AutoConfig.from_pretrained(config.vlm_checkpoint_path, trust_remote_code=True)
            self.vlm_model = Qwen3VLForConditionalGeneration._from_config(vlm_cfg, torch_dtype=self.dtype, attn_implementation="flash_attention_2")
            self.vlm_model.to(device="cuda", dtype=self.dtype)
            logger.info("Initializing VLM from config (flash_attention_2)...")
        ...
        # Store action padding dimensions (for action loss masking)
        # These are zero-padded dims that should NOT contribute to action MSE loss
        self.action_padding_dims = set(getattr(config, 'action_padding_dims', ()) or ())
        if self.action_padding_dims:
            valid_dims = [i for i in range(config.action_dim) if i not in self.action_padding_dims]
            logger.info(f"Action padding dims: {sorted(self.action_padding_dims)}, "
                        f"valid loss dims: {valid_dims} ({len(valid_dims)}/{config.action_dim})")
        ...
        self.freeze_vision_encoder = subtask_cfg.get('freeze_vision_encoder', True)  # default: frozen
    ...
    def load_pretrain_weights(self, path: str) -> None:
        ...
        Skips action_expert.input_encoder and action_expert.decoder so they
        are randomly initialized (not loaded from pretrain checkpoint).

        Also skips duplicate keys (action_module.action_expert.*) to avoid
        state_dict key collisions.
        """
        ...
        # 1. action_module.action_expert.* (duplicate keys from Module wrapping)
        # 2. action_expert.input_encoder.* (don't load action encoder)
        # 3. action_expert.decoder.* (don't load action decoder)
        filtered = {}
        skipped_dup = 0
        skipped_encoder = 0
        skipped_decoder = 0
        for k, v in state_dict.items():
            if k.startswith('action_module.action_expert.'):
                skipped_dup += 1
                continue
            if 'input_encoder' in k:
                skipped_encoder += 1
                continue
            if 'decoder' in k and 'input_encoder' not in k and 'subtask' not in k:
                # Skip action_expert decoder, but keep subtask_text_decoder
                # Only skip if it's under action_expert
                if k.startswith('action_expert.') or k.startswith('action_module.action_expert.'):
                    skipped_decoder += 1
                    continue
            filtered[k] = v
        if skipped_dup > 0:
            logger.info(f"Skipped {skipped_dup} duplicate action_module.action_expert.* keys")
        if skipped_encoder > 0:
            logger.info(f"Skipped {skipped_encoder} input_encoder keys (not loaded from pretrain)")
        if skipped_decoder > 0:
            logger.info(f"Skipped {skipped_decoder} action_expert decoder keys (not loaded from pretrain)")

        # Load weights (strict=False, missing keys will be randomly initialized)
        missing, unexpected = self.load_state_dict(filtered, strict=False)
        logger.info(f"Loaded pretrain weights. "
                     f"Missing: {len(missing)}, Unexpected: {len(unexpected)}")

        # Log specifically whether input_encoder and decoder were loaded
        loaded_enc = [k for k in filtered if 'input_encoder' in k]
        loaded_dec = [k for k in filtered if 'decoder' in k and 'input_encoder' not in k]
        skipped_missing_enc = [k for k in missing if 'input_encoder' in k]
        skipped_missing_dec = [k for k in missing if 'decoder' in k and 'input_encoder' not in k and 'subtask' not in k]
        logger.info(f"  input_encoder keys loaded: {len(loaded_enc)} (skipped: {len(skipped_missing_enc)})")
        logger.info(f"  action_expert decoder keys loaded: {len(loaded_dec)} (skipped: {len(skipped_missing_dec)})")
    ...
    def training_step(
        ...
        # Action loss (exclude padding dimensions from loss)
        if self.action_padding_dims:
            # Only compute MSE on valid (non-padded) dimensions
            # e.g. 16-dim action with padding at [6, 14] → loss on 14 valid dims
            valid_dims = [i for i in range(action_pred.shape[-1])
                          if i not in self.action_padding_dims]
            action_loss = torch.nn.functional.mse_loss(
                action_pred[..., valid_dims],
                action_target[..., valid_dims],
                reduction='mean'
            )
        else:
    ...
    def _compute_subtask_lm_loss(
        ...
        KEY FIX: Freeze Vision Encoder during subtask loss computation.
        - Vision Encoder (model.visual, 407M): FROZEN with torch.no_grad()
          → Same as MoT path (extract_per_layer_features in qwen3_module_wan.py)
          → Prevents subtask CE loss from corrupting visual feature extraction
        - 28-layer LM (language_model, 1720M): TRAINABLE
          → Produces hidden states for both MoT and subtask prediction
        - lm_head (311M): TRAINABLE
          → Maps hidden states to vocabulary for subtask text generation
        ...
        pixel_values = subtask_lm_inputs.get('pixel_values')
        image_grid_thw = subtask_lm_inputs.get('image_grid_thw')
        if pixel_values is not None:
            pixel_values = pixel_values.to(self.device, dtype=self.dtype)
        if image_grid_thw is not None:
            image_grid_thw = image_grid_thw.to(self.device)

        # ---- Manual forward (mirrors MoT path in qwen3_module_wan.py) ----
        # Step 1: Get text embeddings (trainable - shared with MoT path)
        inputs_embeds = self.vlm_model.get_input_embeddings()(input_ids)

        # Step 2: Get image features
        # If freeze_vision_encoder=True: no_grad (prevents subtask CE loss from updating Vision Encoder)
        # If freeze_vision_encoder=False: allow gradients to flow back to Vision Encoder
        if pixel_values is not None and image_grid_thw is not None:
            if self.freeze_vision_encoder:
                with torch.no_grad():
                    image_embeds, deepstack_image_embeds = \
                        self.vlm_model.get_image_features(pixel_values, image_grid_thw)
            else:
                image_embeds, deepstack_image_embeds = \
                    self.vlm_model.get_image_features(pixel_values, image_grid_thw)
            image_embeds = torch.cat(image_embeds, dim=0).to(self.device, self.dtype)

            # Insert image embeddings into inputs_embeds
            ...
            # Text-only subtask (no images)
            deepstack_image_embeds = None
            visual_pos_masks = None
            position_ids = None

        # Step 3: Forward through 28-layer LM (trainable)
        # This produces hidden states that are used for subtask prediction
        vlm_kwargs = {
        ...
        lm_output = self.vlm_model.model.language_model(**vlm_kwargs)
        hidden_states = lm_output.last_hidden_state  # [B, L, 2048]

        # Step 4: Apply lm_head to get logits (trainable)
        logits = self.vlm_model.lm_head(hidden_states)  # [B, L, vocab_size]

        # Step 5: Compute CE loss (shifted for causal LM)
        # logit at position t predicts token at position t+1
        shift_logits = logits[:, :-1, :].contiguous()   # [B, L-1, vocab]
        shift_labels = labels[:, 1:].contiguous()       # [B, L-1]
        subtask_loss = F.cross_entropy(
        ...
        try:
            ...
            if mask.any():
                ...
                if self._subtask_debug_step % 500 == 1:
                    ...
                    logger.info(f"[DEBUG subtask] Vision Encoder FROZEN (no_grad), LM+lm_head trainable")
        except Exception as e:
