...
class RobotWinTaskDataset(data.Dataset):
    ...
    def __init__(
        ...
        subtask_supervision_ratio: float = 0.7,  # Fraction of each phase that gets subtask text supervision
    ):
        """
        ...
        # Subtask supervision ratio: only first N% of each phase gets subtask text labels
        # Last (1-N)% gets all -100 labels (no CE supervision), to avoid boundary confusion
        self.subtask_supervision_ratio = subtask_supervision_ratio
        logger.info(f"Subtask supervision ratio: {subtask_supervision_ratio} "
                    f"(first {subtask_supervision_ratio*100:.0f}% of each phase gets subtask text, "
                    f"last {(1-subtask_supervision_ratio)*100:.0f}% gets no supervision)")
            ...
            try:
                ...
                if phase_path is not None:
                    try:
                        ...
                        # ===== 70/30 split: determine if current frame is in supervised region =====
                        # First N% of each phase gets subtask text labels (CE supervision)
                        # Last (1-N)% gets all -100 labels (no CE supervision, avoids boundary confusion)
                        phase_start = checkpoints[phase_idx - 1] if phase_idx > 0 else 0
                        phase_end = checkpoints[phase_idx] if phase_idx < len(checkpoints) else total_steps
                        phase_length = phase_end - phase_start
                        if phase_length > 0:
                            supervised_end = phase_start + int(phase_length * self.subtask_supervision_ratio)
                        else:
                            supervised_end = phase_start  # degenerate phase, supervise everything
                        # is_supervised = True if current frame is in first 70% of this phase
                        is_subtask_supervised = (condition_frame_idx < supervised_end)

                        # Get subtask text (use same instruction_idx)
                        # subtasks has 100 entries (seen), subtasks_unseen has 100 entries
                        # instruction_idx is in [0, 199] (from 200 umt5_wan embeddings)
                        # Map: 0-99 → subtasks, 100-199 → subtasks_unseen
                        subtasks_unseen_list = phase_data.get('subtasks_unseen', [])
                        if subtasks_list and instruction_idx < len(subtasks_list):
                            task_subtasks = subtasks_list[instruction_idx]
                        elif subtasks_unseen_list and instruction_idx < len(subtasks_list) + len(subtasks_unseen_list):
                            task_subtasks = subtasks_unseen_list[instruction_idx - len(subtasks_list)]
                        else:
                            task_subtasks = None
                                ...
                                if self.vlm_processor is not None:
                                    try:
                                        ...
                                        if target_text:  # Only build if target is non-empty
                                            ...
                                            # Append EOS token so the model learns to STOP generation
                                            eos_id_tok = tokenizer.eos_token_id
                                            if eos_id_tok is not None:
                                                target_ids_full = target_ids_full + [eos_id_tok]
                                            ...
                                            # Append EOS token after the target text so model learns to stop
                                            ...
                                            # ===== 70/30 split: mask labels for last 30% of phase =====
                                            # If current frame is in the last 30% of this phase,
                                            # set ALL labels to -100 (no CE supervision).
                                            # This avoids boundary confusion where the robot may already
                                            # be transitioning to the next subtask but phase_idx hasn't updated.
                                            # VLM still processes the text (forward pass), but gets no gradient.
                                            if not is_subtask_supervised:
                                                # mot_decoder labels: all -100
                                                subtask_dec_labels = torch.full_like(subtask_dec_labels, -100)
                                                # native_lm labels: all -100
                                                subtask_lm_inputs['labels'] = torch.full_like(
                                                    subtask_lm_inputs['labels'], -100
                                                )
